...
export function ChatReactions({ message, actor, onReact, disabled = false }: { message: StoredChatMessage; actor: ChatActor; onReact: (id: string, emoji: string) => Promise<void>; disabled?: boolean }) {
  ...
  return <div className="chat-reactions" onKeyDown={(event) => {
    ...
    <div className="chat-reaction-summary">
      ...
      {!disabled && <button type="button" className="chat-reaction-add" aria-label="Thả cảm xúc" aria-expanded={open} title="Thả cảm xúc" onClick={() => { setOpen(!open); setShowAll(false); }}><SmilePlus size={15} /></button>}
    </div>
    {open && !disabled && <div className="chat-reaction-picker" aria-label="Chọn cảm xúc cho tin nhắn">
